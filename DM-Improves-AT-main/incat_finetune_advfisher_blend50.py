...
from core.utils import Logger, Trainer, ctx_noparamgrad_and_eval, format_time, parser_train, seed
...
def _estimate_fisher(model, dataloader, max_batches=100, attack=None):
    model.eval()
    ...
    for batch_idx, (x, y) in enumerate(dataloader):
        ...
        if attack is not None:
            # Build adversarial views for Fisher without updating model parameters.
            with ctx_noparamgrad_and_eval(model):
                x, _ = attack.perturb(x, y)
...
def compute_blended_fisher_diagonal(model, dataloader, attack, max_batches=100, alpha=0.5):
    clean_fisher = _estimate_fisher(model, dataloader, max_batches=max_batches, attack=None)
    adv_fisher = _estimate_fisher(model, dataloader, max_batches=max_batches, attack=attack)

    blended = {}
    for name in clean_fisher:
        blended[name] = alpha * clean_fisher[name] + (1.0 - alpha) * adv_fisher[name]
    return blended
...
def main():
    ...
    parser.add_argument('--fisher-clean-weight', type=float, default=0.5, help='Weight for clean Fisher in blended Fisher (0.5 => clean/adv 50/50).')
    ...
    logger.log(
        'INCAT objective: adversarial loss + blended Fisher '
        '(fisher_lambda={}, clean_weight={:.2f}, adv_weight={:.2f})'.format(
            args.fisher_lambda, args.fisher_clean_weight, 1.0 - args.fisher_clean_weight
        )
    )

    fisher_loader = load_fisher_loader(args)
    logger.log('Estimating blended Fisher diagonal (clean + adversarial)...')
    fisher_diag = compute_blended_fisher_diagonal(
        trainer.model,
        fisher_loader,
        attack=trainer.attack,
        max_batches=args.fisher_batches,
        alpha=args.fisher_clean_weight,
    )
    reference = {name: p.detach().clone() for name, p in trainer.model.named_parameters() if p.requires_grad}
    logger.log('Blended Fisher estimation done.')
